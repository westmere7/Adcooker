...
// Drives Auto-arrange ▸ Clear placement ▸ All projects; "Forget all" below is the
// everything-everywhere version behind Settings.
async function forgetPlacementsFromLibrary(width, height, roles) {
  if (!sb) throw new Error('Cloud is not configured.');
  if (!authState.currentUser()) throw new Error('Not signed in.');

  // Re-read before writing so a placement saved on another machine is not dropped by this
  // one's cache, exactly as the save path does.
  const remote = await fetchPlacementLibrary();
  ...
  // Never leave an empty size bucket behind — it would count as a remembered size in the
  // Settings summary while holding nothing.
