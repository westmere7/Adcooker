...
const CHANGELOG_DATA = [
  {
    ...
    items: [
      ...
      'The Preview Takes a Normal Right-Click: it is a real image (or video) element, so the browser\'s own menu now opens on it — Save image as…, Copy image, Open in new tab — instead of the editor\'s canvas menu, which had nothing useful to offer there. Right-clicking anywhere else in the app is unchanged.',
      ...
      'Copy a GIF to the Clipboard: a Copy button beside Download puts the GIF straight on the clipboard for pasting into Slack, Teams, a deck or an email. Worth knowing why it has a caveat: the Clipboard API only requires browsers to accept PNG for images, and they sanitise and re-encode whatever they are given — which flattens animation. Where a browser refuses an animated GIF (Chrome does today), Copy falls back to the frame on screen as a still and says so plainly, so you never think you pasted an animation when you pasted a frame. Download always gives the real animated file.',
